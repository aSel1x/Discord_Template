...
from app.utils.singlethon import SingletonMeta
...
class PostgresDB(metaclass=SingletonMeta):
    ...
    async def __set_repositories(self) -> None:
        if self.__session_maker is not None:
            ...
            self.wallet = repos.WalletRepo(self.__session_maker)
            self.transaction = repos.TransactionRepo(self.__session_maker)
